...
function UpdateChanllenge() {
  ...
  useEffect(() => {
    (async () => {
      ...
      let indexStart = chanllenge.defaultCode[0].code.indexOf(" ") + 1;
      let indexEnd = chanllenge.defaultCode[0].code.indexOf("(");
      let functionName = chanllenge.defaultCode[0].code.substring(
        indexStart,
        indexEnd
      );
      chanllenge.functionName = functionName;
      chanllenge.typeOutput = Array.isArray(chanllenge.testcases[0].output)
        ? "array"
        : typeof chanllenge.testcases[0].output;
      //Testcases
      let testcasesFormat = chanllenge.testcases.map((testcase) => {
        delete testcase._id;
        return {
          ...testcase,
        };
      });
      setNumberAgrs(testcasesFormat[0].input.length + "");
      chanllenge.testcases = JSON.stringify(testcasesFormat, null, "\t");
  ...
  const onSubmitForm = (data) => {
    ...
    try {
      ...
      ChanllengeApi
      .updateChanllenge(id,chanllenge)
      .then((data) => {
        setDialog({
          ...dialog,
          message: "Update chanllenge success!",
          isSuccess: true,
          isOpen: true,
        });
      ...
      .catch((err) => {
        ...
        setDialog({
          ...dialog,
          message: err.response.data.error || err.response.data.err,
          isSuccess: false,
          isOpen: true,
        });
      });
      ...
      let layout = (
        ...
        >
          ...
          <select
            ...
            defaultValue={chanllenge.typeInput[index]}
          >
                ...
                <label className="w-full block text-grey-darker text-sm mb-1 mt-3">
                  ...
                  {difficults && difficults.length > 0 && (
                    <select
                      {...register("difficult")}
                      className="w-full px-2 py-1 bg-white border shadow-sm border-slate-300 placeholder-slate-400 focus:outline-none focus:border-primary focus:ring-primary block rounded-md sm:text-sm focus:ring-1"
                      defaultValue={chanllenge.difficult._id}
                      onChange={handleOnchange}
                    >
            ...
            <div className="flex gap-x-3">
              ...
              <div className="w-1/2">
                <label className="w-full block text-grey-darker text-sm mb-1 mt-3">
                  ...
                  <select
                    defaultValue={chanllenge.typeOutput}
                    onChange={handleOnchange}
                    {...register(`typeOutput`)}
                    className="w-full px-2 py-1 bg-white border shadow-sm border-slate-300 placeholder-slate-400 focus:outline-none focus:border-primary focus:ring-primary block rounded-md sm:text-sm focus:ring-1"
                  >
                ...
                <label className="w-full block text-grey-darker text-sm mb-1 mt-3">
                  ...
                  <div className="flex gap-x-5">
                    ...
                    {chanllenge.typeOutput !== 'object' &&
                    <div className="">
                    {chanllenge.defaultCode[1] ? (
                      <input
                        checked={true}
                        type="checkbox"
                        {...register(`py`)}
...
export default UpdateChanllenge;
